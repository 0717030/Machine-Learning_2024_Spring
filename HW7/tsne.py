...
def perform_tsne_ssne(X=np.array([]), no_dims=2, initial_dims=50, perplexity=30.0, mode=0,title=""):
    """
    ...
    for iter in range(max_iter):
        ...
        if (iter + 1) % 10 == 0:
            ...
            img = get_current_image(Y, labels,mode,iter+1)
            img_list.append(img)
    ...
    visualize_similarity(P,Q,title)
...
if __name__ == "__main__":
    ...
    for i in range(1,-1,-1):
        for p in range(len(perplexity)):
            ...
            plt.clf()
            Y = perform_tsne_ssne(X, 2, 50, perplexity[p], i,title)
